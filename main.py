...
if __name__ == "__main__":
    asyncio.run(main())            "أرسل النص الذي تريد نشره\n\n"
        ...
        if not groups:
            await callback.answer("❌ أضف جروباً أولاً!", show_alert=True)
            return
        if not captions:
            await callback.answer("❌ أضف كليشة أولاً!", show_alert=True)
            return
        if not users[user_id].get("session"):
            await callback.answer("❌ سجل دخولك أولاً! أرسل /login", show_alert=True)
            return
        ...
        await callback.message.edit_text(
            ...
            f"📝 عدد الكليشات: {len(captions)}\n"
            f"⏱ المدة: {users[user_id]['time']} ثانية\n\n"
            f"🔄 جاري النشر...",
            reply_markup=main_menu(user_id)
        )
        
        # بدء مهمة النشر
        asyncio.create_task(start_posting(client, int(user_id)))
        await callback.answer()
    
    elif data == "stop":
        users[user_id]["active"] = False
        save_users(users)
        await callback.message.edit_text("⏹ **تم إيقاف النشر**", reply_markup=main_menu(user_id))
        await callback.answer()
    
    elif data == "status":
        g = len(users[user_id].get("groups", []))
        c = len(users[user_id].get("captions", []))
        t = users[user_id].get("time", 60)
        d = users[user_id].get("delete", 0)
        a = "🟢 يعمل" if users[user_id].get("active") else "🔴 متوقف"
        s = "✅ موجود" if users[user_id].get("session") else "❌ غير مسجل"
...
# =================== استقبال الرسائل (إضافة البيانات) ===================
@app.on_message(filters.private & ~filters.command("start") & ~filters.command("login"))
async def handle_input(client, message):
    user_id = str(message.from_user.id)
    text = message.text
    
    if text == "/cancel":
        await message.reply("❌ تم الإلغاء", reply_markup=main_menu(user_id))
        return
    
    # نتحقق من آخر حالة للمستخدم
    # هذا تبسيط - الأفضل حفظ حالة لكن للتجربة سنستخدم طريقة بسيطة
    
    # محاولة كرقم جروب
    if text.startswith("-") and text.lstrip("-").isdigit():
        if "groups" not in users[user_id]:
            users[user_id]["groups"] = []
        if text not in users[user_id]["groups"]:
            users[user_id]["groups"].append(text)
            save_users(users)
            await message.reply(f"✅ تم إضافة الجروب `{text}`", reply_markup=main_menu(user_id))
        else:
            await message.reply("⚠️ هذا الجروب مضاف بالفعل", reply_markup=main_menu(user_id))
    
    # محاولة ككليشة
    elif len(text) > 5:
        if "captions" not in users[user_id]:
            users[user_id]["captions"] = []
        users[user_id]["captions"].append(text)
        save_users(users)
        await message.reply(f"✅ تم إضافة الكليشة\n\n{text[:100]}", reply_markup=main_menu(user_id))
    
    # محاولة كرقم (مدة)
    elif text.isdigit():
        num = int(text)
        # تخمين: إذا كان الرقم بين 10 و 1000 فهو مدة
        if 10 <= num <= 1000:
            users[user_id]["time"] = num
            save_users(users)
            await message.reply(f"✅ تم تعيين المدة إلى {num} ثانية", reply_markup=main_menu(user_id))
        elif num == 0 or 10 <= num <= 86400:
            users[user_id]["delete"] = num
            save_users(users)
            if num == 0:
                await message.reply(f"✅ تم تعطيل الحذف التلقائي", reply_markup=main_menu(user_id))
            else:
                await message.reply(f"✅ سيتم حذف الرسائل بعد {num} ثانية", reply_markup=main_menu(user_id))
        else:
            await message.reply("❌ رقم غير صالح", reply_markup=main_menu(user_id))
    
    else:
        await message.reply("❌ لم أستطع التعرف على المدخل", reply_markup=main_menu(user_id))
...
async def login(client, message):
    ...
    await message.reply(
        "🔐 **تسجيل الدخول**\n\n"
        "هذه نسخة تجريبية مبسطة.\n"
        "للتبسيط، يمكنك وضع أي نص كجلسة.\n\n"
        "أرسل كلمة `session` لتفعيل الحساب الوهمي\n\n"
        "أو ارسل /cancel للإلغاء"
    )
    ...
    if response.text == "/cancel":
        await response.reply("❌ تم الإلغاء", reply_markup=main_menu(user_id))
    else:
        users[user_id]["session"] = "dummy_session"
        save_users(users)
        await response.reply("✅ تم تسجيل الدخول بنجاح!", reply_markup=main_menu(user_id))

# =================== دالة النشر ===================
async def start_posting(client, user_id):
    user_id_str = str(user_id)
    
    while users[user_id_str].get("active", False):
        groups = users[user_id_str].get("groups", [])
        captions = users[user_id_str].get("captions", [])
        wait_time = users[user_id_str].get("time", 60)
        delete_after = users[user_id_str].get("delete", 0)
        ...
        for group in groups:
            if not users[user_id_str].get("active"):
                break
            
            caption = captions[0]  # أول كليشة للتبسيط
            try:
                # محاولة إرسال الرسالة
                msg = await client.send_message(int(group), caption)
                
                # جدولة الحذف
                if delete_after > 0:
                    asyncio.create_task(delete_message(msg, delete_after))
                
                await client.send_message(user_id, f"✅ تم الإرسال إلى {group}")
            except Exception as e:
                await client.send_message(user_id, f"❌ فشل الإرسال إلى {group}: {str(e)[:50]}")
...
async def main():
    ...
    await app.start()
    print("✅ البوت يعمل! أرسل /start في التليجرام")
